...
export class WaveformGrid {
  ...
  updateGrid(time) {
    ...
    const scaledIntensity = Math.min(1, this.intensity) * this.config.waveAmplitude;
    
    // Update AWaves custom properties
    this.wavesElement.style.setProperty('--wave-amplitude', scaledIntensity.toFixed(2));
    this.wavesElement.style.setProperty('--beat-intensity', this.beatIntensity.toFixed(2));
    
    // Different frequencies affect different aspects of the grid
    this.wavesElement.style.setProperty('--bass-level', this.audioFeatures.bass.toFixed(2));
    this.wavesElement.style.setProperty('--mid-level', this.audioFeatures.lowMid.toFixed(2));
    this.wavesElement.style.setProperty('--high-level', this.audioFeatures.treble.toFixed(2));
    this.wavesElement.style.setProperty('--transient-level', this.audioFeatures.transientEnergy.toFixed(2));
    ...
    if (this.config.frequencyScale > 0) {
      ...
      const logFreq = Math.log(Math.max(20, Math.min(20000, this.audioFeatures.peakFrequency || 1000)));
      const normalizedFreq = (logFreq - logMin) / (logMax - logMin);
      
      this.wavesElement.style.setProperty('--freq-normalized', normalizedFreq.toFixed(2));
    ...
    if (this.config.dynamicDetail) {
      ...
      this.wavesElement.style.setProperty('--detail-level', detailLevel.toFixed(2));
    }
  }
  ...
  pulse(intensity) {
    ...
    if (intensity > 0) {
      ...
      this.wavesElement.style.setProperty('--pulse-intensity', intensity.toFixed(2));
    } else {
